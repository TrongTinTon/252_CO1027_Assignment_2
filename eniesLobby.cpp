...
void clampCharacter(Character* c) {
    ...
    if (c->getDef() < 0) c->setDef(0);
    if (c->getSpeed() < 0) c->setSpeed(0);
}
...
int applyDamage(Character* attacker, Character* target, int rawDamage,
    ...
    if (!attacker || !target || !target->isAlive()) return 0;
...
int Luffy::attack(Character* target, BattleContext& context) {
    ...
    if (!hpGreaterPercent(hp, maxHp, 50)) {
        if (hpGreaterPercent(hp, maxHp, 30)) dmg = ceilPercentValue(atk, 115);
        else dmg = ceilPercentValue(atk, 130);
    }
    return applyDamage(this, target, dmg, context);
}

int Luffy::specialSkill(Character* target, BattleContext& context) {
    if (energy < 20 || !hpGreaterEqualPercent(hp, maxHp, 15)) return 0;
...
void Luffy::endTurn(BattleContext& context) {
    if (hpLessEqualPercent(hp, maxHp, 30)) context.morale += 3;
...
int Zoro::attack(Character* target, BattleContext& context) {
    ...
    if (target && hpLessPercent(target->getHP(), target->getMaxHP(), 40)) dmg = ceilPercentValue(dmg, 115);
    return applyDamage(this, target, dmg, context);
}
...
int Zoro::specialSkill(Character* target, BattleContext& context) {
    ...
    if (target && hpLessPercent(target->getHP(), target->getMaxHP(), 50)) dmg = ceilPercentValue(dmg, 150);
...
int Zoro::specialSkill(Building* target, BattleContext& context) {
    ...
    int dmg = ceilPercentValue(atk, 220);
    if (target && target->getHP() * 100 < target->getMaxHP() * 50) dmg = ceilPercentValue(dmg, 150);
    return applyDamageBuilding(this, target, dmg, context);
}
...
int Sanji::attack(Character* target, BattleContext& context) {
    ...
    if (target && target->getDef() < def) dmg = ceilPercentValue(dmg, 110);
    return applyDamage(this, target, dmg, context);
}
...
int Chopper::specialSkill(Character* target, BattleContext& context) {
    if (energy < 15 || !target || !target->isStrawHat() || !target->isAlive()) return 0;
    energy -= 15;
...
int Usopp::attack(Character* target, BattleContext& context) {
    int dmg = atk;
    if (target && target->getSpeed() < 50) dmg = ceilPercentValue(dmg, 120);
    return applyDamage(this, target, dmg, context);
}
...
int Franky::attack(Character* target, BattleContext& context) {
    ...
    if (target && target->isCP9()) dmg = ceilPercentValue(dmg, 110);
    return applyDamage(this, target, dmg, context);
}
...
int Franky::specialSkill(Character* target, BattleContext& context) {
    ...
    if (energy >= 20) {
        ...
        clampCharacter(target);
        if (target->getName() == "Lucci") dmg = ceilPercentValue(dmg, 120);
        int actual = applyDamage(this, target, dmg, context);
        clampCharacter(this);
        return actual;
    }
...
void Franky::endTurn(BattleContext& context) {
    if (hpGreaterPercent(hp, maxHp, 70)) def += 5;
    if (hpLessPercent(hp, maxHp, 30)) atk += ceilPercentValue(atk, 10);
    clampCharacter(this);
}
...
int Lucci::attack(Character* target, BattleContext& context) {
    ...
    if (target && hpLessPercent(target->getHP(), target->getMaxHP(), 50)) dmg = ceilPercentValue(dmg, 120);
    return applyDamage(this, target, dmg, context);
}
...
void Lucci::endTurn(BattleContext& context) {
    if (hpLessPercent(hp, maxHp, 40)) atk += ceilPercentValue(atk, 5);
    clampCharacter(this);
}
...
int Kaku::specialSkill(Character* target, BattleContext& context) {
    ...
    if (target && target->isAlive()) total += applyDamage(this, target, atk, context);
    if (target && target->isAlive()) total += applyDamage(this, target, ceilPercentValue(atk, 80), context);
    return total;
}
...
int Jabra::specialSkill(Character* target, BattleContext& context) {
    ...
    if (hpLessPercent(hp, maxHp, 30)) dmg = ceilPercentValue(dmg, 125);
...
int Blueno::specialSkill(Character* target, BattleContext& context) {
    ...
    if (hpGreaterPercent(hp, maxHp, 50)) dmg += 20;
    else dmg += 40;
    return applyDamage(this, target, dmg, context);
}
...
int Kumadori::specialSkill(Character* target, BattleContext& context) {
    ...
    if (hpLessPercent(hp, maxHp, 40)) dmg += 25;
    return applyDamage(this, target, dmg, context);
}
...
int Fukurou::specialSkill(Character* target, BattleContext& context) {
    ...
    int dmg = ceilPercentValue(atk, 130);
    if (target && target->isLowestHP_Enemies) dmg += 20;
...
void EniesLobbyBattle::addStrawHat(Character* character) {
    if (character && strawHatCount < 7) strawHats[strawHatCount++] = character;
}

void EniesLobbyBattle::addCP9Agent(Character* character) {
    if (character && cp9Count < 7) cp9Agents[cp9Count++] = character;
}

void EniesLobbyBattle::addBuilding(Building* building) {
    if (building && buildingCount < 5) buildings[buildingCount++] = building;
}
...
void EniesLobbyBattle::processTurn(Character* character) {
    ...
    if (n == "Luffy") useSpecial = (character->getEnergy() >= 20 && hpGreaterEqualPercent(character->getHP(), character->getMaxHP(), 15));
